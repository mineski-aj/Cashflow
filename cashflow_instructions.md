...
**Version:** 2.5 | **Last updated:** 2026-09-16

 

**Changelog:**

- v2.5 (2026-09-16) — **Bug 12: `update_cashflow.py`'s Jul–Dec `FY_BEG_INIT`/`FY_NET_INIT` were built from a hand-rolled chain, not read from CF for Mancom — and that chain had a sign bug on top.** Right after v2.4 removed the AP-hybrid from `FullYear()` in `index.html`, the December closing still didn't match CF for Mancom's own row 67 (dashboard showed +₱285.2M; CF's own W52 column showed −₱66.3M). Root cause was in the *script*, not the frontend: `fy_beg_fut`/`fy_net_fut` (Jul–Dec) were never reading CF row 49/67 directly the way Jan–Jun does (`beg_jan = cfv(49, cf_month_cols['jan'][0])` etc.) — instead they reconstructed a running balance via `b = b + inflow - outflow`, where `outflow` was built from the **AP sheet's H2 schedule** (`fy_ap_tot_arr`, i.e. `fy_ap_tot_h2`) rather than `FY_COS_CF` (CF row 57), and was assigned as `outflow = -(sum of positive cost figures)` — already negative — so `b - outflow` **added** every cost back into the balance instead of subtracting it. Two bugs stacked: wrong source, wrong sign. **Fix:** replaced the whole hand-rolled chain with a direct read of CF row 49 (first column of the month) / row 67 (last column of the month) for Jul–Dec, via a new `cf_month_cols_full` mapping that concatenates `act_cols + proj_cols` per month name — the exact same rule Jan–Jun already used, just extended past June. This can't diverge from CF for Mancom by construction, matching the v2.4 "100% CF for Mancom" intent for Beginning/Closing specifically (the flow rows — COS/GAE/Tax/CAPEX/Loan/Other/GG Outflow — were already correct; only Beginning/Closing were wrong). **Verification:** `FY_NET_INIT[11]` now equals `cfv(67, <last Dec column>)` exactly (−₱66,315,858 on the W38 file) — always spot-check this against CF for Mancom's own last-week Net Cash Inflow (Outflow) value after running the script.

- v2.4 (2026-09-16) — **Full Year is now 100% CF for Mancom, arranged monthly — the AP-sheet/AP_PAYABLES hybrid is removed from this tab entirely (by user request).** Previously, Jul–Dec's "Payables 2026 / AP" row combined CF row 57 (COS) with an optional add-on of `AP_PAYABLES`' hand-curated 2026-dated Cost of Sales rows (toggle "AP 2026 schedule: ON/OFF", defaulted ON), and two more rows ("Payables 2024"/"Payables 2025") injected `AP_PAYABLES`' prior-year arrears on top of the CF-derived chain (toggle "Hiding prior-year payables"). Both were flagged as a real double-count risk in the W38 update: `AP_PAYABLES` still carried ~₱4.13M of MPL PH S17-family vendor bills that were also now flowing through CF's own project-level cost liquidity plan after that week's xlsx edit. **Fix:** `FullYear()` in `index.html` now uses `FY_BEG_INIT`/`FY_NET_INIT`/`FY_PDEI_IN_INIT`/`FY_GG_IN_INIT`/`FY_COS_CF`/`FY_GAE`/`FY_TAX`/`FY_CAPEX`/`FY_LOAN`/`FY_OTHER`/`FY_GG_OUT`/`FY_FOREX` directly, with no override, for all 12 months — these were already 100%-CF-derived for the whole year by `update_cashflow.py` per Bug 7/9's fixes, so no script changes were needed, only the frontend hybrid layer was removed. Removed: `inclAP2026`/`inclHist` toggles and their buttons, the "Payables 2024"/"Payables 2025" rows, the Jul–Dec BEG/NET recompute chain (CF's own row 49/67 values are used as-is — no re-derivation), and the corresponding breakdown popovers. The "Payables 2026 / AP" row is renamed "COS (Cost of Sales)" `[CF]`, click-to-drill now shows a single line ("CF for Mancom — projected COS (row 57)") with no AP items. **`AP_PAYABLES`, `AP_COS_VENDORS`, and `ARREARS_VENDORS` are untouched** — they still back the separate "AP Payables" and "AP Arrears" tabs (`APPayables()`/`ArrearsTab()`), which are unrelated to Full Year and were not part of this change. **Historical note:** Step 5B/5C/5D below (AP-sheet-driven Jul–Dec Full Year construction) is now superseded for the Full Year tab specifically — do not follow it for Full Year going forward. It's left in place only because Step 5C's "AP Arrears" content (the separate 9th-slide tab) still applies.
...
> ⚠ **SUPERSEDED v2.4 (2026-09-16) for Full Year specifically.** Full Year is now 100% CF for Mancom for all 12 months — `FY_BEG_INIT`/`FY_NET_INIT`/`FY_PDEI_IN_INIT`/`FY_GG_IN_INIT`/`FY_COS_CF`/`FY_GAE`/`FY_TAX`/`FY_CAPEX`/`FY_LOAN`/`FY_OTHER`/`FY_GG_OUT`/`FY_FOREX` straight from `update_cashflow.py`, no AP sheet, no `AP_PAYABLES`, no toggles. Steps 5A/5B/5C (below) describing an AP-sheet-driven Jul–Dec zone no longer apply to Full Year — kept only for history and because 5C's AP Arrears tab content is still current. See changelog v2.4 for the full rationale.

 

**Three-zone column layout (historical — no longer used for Full Year, see note above):**
